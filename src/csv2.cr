...
# ### Building
#
# To create CSV data, check `CSV#build` and the `CSV2::Builder` class.
class CSV2
  DEFAULT_SEPARATOR  = ','
  DEFAULT_QUOTE_CHAR = '"'
  ...
  # Appends CSV data to the given `IO`. This yields a `CSV2::Builder`
  ...
  # Raises `CSV2::Error` if headers were not requested.
  ...
  # A Row of a `CSV2::WithHeaders` instance.
  struct Row
    private getter csv : CSV2
    @row : Array(String)
